...
| `axehero-buzz` | `axehero-buzz` | 3399 | Multi-service stack (relay + postgres + redis + minio). Uses `app_proxy` with `PROXY_AUTH_ADD: "false"` — Umbrel's password gate would break the NIP-42 WebSocket handshake; access control is delegated to the relay's closed mode. An `init` service chowns `config/` and `git/` to 1000:1000 before the relay starts, because Docker creates missing bind-mount targets as root. First-boot secrets are generated by `entrypoint.sh` into `${APP_DATA_DIR}/config/buzz.env`, which is never overwritten on update. Changing `BUZZ_PUBLIC_HOST` after data exists seeds a new, empty community. |
